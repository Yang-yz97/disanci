...
<template>
  <div class="teams">
    ...
    <div class="search-bar mb-20">
      <el-form :inline="true" :model="searchForm" class="form-inline">
        <el-form-item label="领用人">
          <el-input v-model="searchForm.code" placeholder="请输入领用人" clearable />
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="code" label="领用人" width="120" />
      <el-table-column prop="name" label="团队名称" min-width="150" show-overflow-tooltip />
    ...
    >
      <el-form
        ...
        class="team-form"
      >
        <el-form-item label="领用人" prop="code">
          <el-input v-model="form.code" placeholder="请输入领用人" :disabled="dialogType === 'edit'" />
        </el-form-item>
...
const handleBatchDelete = async () => {
  ...
  try {
    const ids = selectedTeams.value.map(row => row.id)
    await customerService.batchDeleteCustomers(ids)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.error(dialogType.value === 'add' ? '新增团队失败:' : '更新团队失败:', error)
...
const handleExport = async () => {
  try {
    ...
    link.download = `团队列表_${new Date().toLocaleString()}.xlsx`
    link.click()
    URL.revokeObjectURL(link.href)
  } catch (error) {
    console.error('导出团队失败:', error)
    ElMessage.error('导出团队失败')
  }
}
